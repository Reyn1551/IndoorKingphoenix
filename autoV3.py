...
#####################################################
##    KING PHOENIX V7: AUTO SWEEP SEARCHING QR     ##
#####################################################
...
DEFAULT_CONFIG = {
    ...
    "vision": {
        ...
        "lost_timeout": 3.0,        
        "camera_offset_x": 0,
        # --- FITUR BARU: AUTO SWEEP ---
        "scan_mode": "SWEEP",       # "NONE" (Diam), "FIXED" (Geser 1 Arah), "SWEEP" (Kiri-Kanan)
        "scan_sweep_amp": 0.15,     # Kecepatan geser maksimal (m/s) -> Menentukan lebar sapuan
        "scan_sweep_speed": 1.5,    # Kecepatan ayunan (Makin besar makin cepat bolak-balik)
        "qr_confirm_count": 1       
    },
    "t265": {
        "scale_factor": 1.0,        
...
PARAM_DESCRIPTIONS = {
    "scan_mode": "Pilih: 'NONE' (Diam), 'FIXED' (Geser lurus), 'SWEEP' (Goyang Kiri-Kanan).",
    "scan_sweep_amp": "Lebar ayunan (m/s). 0.1 = geser dikit, 0.2 = geser lebar.",
    "scan_sweep_speed": "Frekuensi ayunan. 1.0 = Lambat, 3.0 = Cepat pusing.",
    "takeoff_alt": "Target ketinggian saat takeoff (meter).",
    "forward_speed": "Kecepatan maju konstan (m/s).",
    "gain_yaw": "Sensitivitas koreksi belok (PID P-Gain).",
    "max_lat_vel": "Limit kecepatan menyamping (safety).",
    "rotation_angle": "Sudut putar balik (U-Turn).",
    "rotation_time": "Durasi putaran (detik).",
    "land_after_mission": "Auto Land setelah pulang?",
    "alt_source": "Sumber: 'T265' atau 'FCU' (Baro/Lidar).",
    "is_black_line": "Centang jika garis HITAM di lantai PUTIH.",
    "threshold_val": "Sensitivitas cahaya (0-255).",
    "roi_height_ratio": "Crop atas kamera (0.0=Full, 0.5=Setengah).",
    "min_area": "Filter ukuran objek (pixel).",
    "min_aspect_ratio": "Filter bentuk garis.",
    "lost_timeout": "Batas waktu garis hilang sebelum dianggap ujung.",
    "camera_offset_x": "Geser target tengah (Garis Biru).",
    "qr_confirm_count": "Berapa kali QR terbaca sebelum putar (1=Instan).",
    "scale_factor": "Kalibrasi skala T265.",
    "confidence_threshold": "Level T265 minimal (3=High).",
    "ignore_quality": "BAHAYA: Tetap terbang meski T265 Low/Failed.",
    "qr_keyword": "Teks QR Code untuk memicu Return.",
    "http_port": "Port Web Server.",
    "mavlink_connect": "String koneksi MAVLink."
}
...
qr_detect_counter = 0 
fcu_altitude = 0.0
scan_start_time = 0.0 # Variable untuk timer gerakan sinus
...
# ==========================================
# 4. MISSION LOGIC (AUTO SWEEP)
# ==========================================
current_vx = 0.0; current_vy = 0.0

def send_vel_cmd():
    global mission_state, current_vx, current_vy, scan_start_time
    
    # 1. TRACKING
    if mission_state in [STATE_OUTBOUND, STATE_INBOUND]:
        vx = current_vx if line_detected else 0.0
        vy = current_vy if line_detected else 0.0
    ...
    # 2. SCANNING (FITUR SWEEP)
    elif mission_state == STATE_SCANNING:
        scan_vy = 0.0
        mode = CONFIG["vision"].get("scan_mode", "SWEEP")
        
        if mode == "SWEEP":
            # Gerakan Sinusoidal (Kiri-Kanan Halus)
            elapsed = time.time() - scan_start_time
            amp = CONFIG["vision"].get("scan_sweep_amp", 0.15)
            freq = CONFIG["vision"].get("scan_sweep_speed", 1.5)
            # Rumus Sinus: Vy berubah harmonis
            scan_vy = amp * m.sin(elapsed * freq)
            
        elif mode == "FIXED":
            # Geser lurus (misal: selalu ke kanan)
            scan_vy = CONFIG["vision"].get("scan_sweep_amp", 0.05)
            
        # Kirim perintah geser (Vy)
        conn.mav.set_position_target_local_ned_send(0, conn.target_system, conn.target_component,
            mavutil.mavlink.MAV_FRAME_BODY_NED, 0b0000111111000111, 0,0,0, 0, scan_vy, 0, 0,0,0, 0,0)
...
def mission_logic_thread():
    global mission_state, qr_data, last_line_time, mission_start_command, qr_detect_counter, scan_start_time
    
    state_desc = [
        "MENUNGGU T265", "MENUNGGU TOMBOL START", "SEDANG TAKEOFF", 
        "MENGIKUTI GARIS (PERGI)", "CARI QR (SWEEPING)", "PUTAR BALIK (U-TURN)", 
        "MENGIKUTI GARIS (PULANG)", "SEDANG MENDARAT"
    ]
    ...
    while True:
        ...
        if mission_state == STATE_INIT:
            ...
            progress("TAKEOFF SELESAI. MULAI JALAN."); mission_state = STATE_OUTBOUND

        elif mission_state == STATE_OUTBOUND:
            if not line_detected and (time.time() - last_line_time > CONFIG["vision"]["lost_timeout"]):
                progress("GARIS HABIS. MULAI SWEEPING..."); 
                mission_state = STATE_SCANNING; qr_data = ""; qr_detect_counter = 0
                scan_start_time = time.time() # Mulai timer untuk gerakan sinus

        elif mission_state == STATE_SCANNING:
            if CONFIG["system"]["qr_keyword"] in qr_data:
                qr_detect_counter += 1
                required_count = CONFIG["vision"].get("qr_confirm_count", 1)
                
                if qr_detect_counter >= required_count:
                    progress(f"QR DITEMUKAN. PUTAR BALIK..."); 
                    mission_state = STATE_ROTATING
                    perform_rotation()
                    time.sleep(CONFIG["flight"]["rotation_time"])
                    last_line_time = time.time(); progress("KEMBALI PULANG..."); mission_state = STATE_INBOUND
                else:
                    qr_data = "" 
            ...
            if CONFIG["flight"]["land_after_mission"]:
...
def gen_frames():
    ...
    while True:
        ...
        for cnt in contours:
            ...
            if (float(bh)/bw) > CONFIG["vision"]["min_aspect_ratio"]:
        ...
        if best_cnt is not None:
            ...
            if M['m00'] > 0:
                ...
                raw_vy = error_x * CONFIG["flight"]["gain_yaw"]
        ...
        cv2.putText(combined, f"MISI: {web_data['state']}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
        cv2.putText(combined, f"ALT: {web_data['alt']:.2f}m", (w+10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
...
<html>
<head>
    <title>PHOENIX MISSION V7</title>
    <style>
        ...
        setInterval(()=>{
            if(document.getElementById('hud').classList.contains('active')){
                fetch('/status').then(r=>r.json()).then(d=>{
                    ...
                    if(d.state.includes("MENUNGGU") && document.getElementById('btn-start').innerText!=="RUNNING...") document.getElementById('btn-start').disabled=false;
                });
            }
        },500);
