...
def gen_frames():
    ...
    while True:
        ...
        for obj in qr_objects:
            ...
            cv2.polylines(frame, [pts], True, (255,0,255), 2); cv2.putText(frame, qr_text, (obj.rect.left, obj.rect.top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,255), 2)

        thresh_color = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR); right_panel = np.zeros_like(frame); right_panel[cut_y:h, 0:w] = thresh_color
        cv2.putText(right_panel, "AI VIEW", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1); cv2.line(right_panel, (cx_scr, 0), (cx_scr, h), (255, 0, 0), 2) 
        combined = cv2.hconcat([frame, right_panel]); cv2.rectangle(combined, (0, 0), (640, 60), (0,0,0), -1) 
        ...
        action_txt = "HOLD"
        if mission_state in [STATE_HOVER_QR, STATE_ROTATING]: action_txt = "ROTATING"
